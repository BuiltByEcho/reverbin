...
test('agent docs describe behavior contracts and safe handling', () => {
  ...
  assert.match(docs, /Attachment and image handling/);
  assert.match(docs, /stored as metadata rows plus authenticated storage links/);
  assert.match(docs, /does not expose public unauthenticated attachment URLs/);
...
test('API docs describe endpoints, events, approvals, and SDK usage', () => {
  ...
  assert.match(docs, /Attachment and image storage/);
  assert.match(docs, /message_attachments/);
  assert.match(docs, /GET `\/mail\/attachments\/:id`/);
  assert.match(docs, /Images render inline in the authenticated mail console/);
